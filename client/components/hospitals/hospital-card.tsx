...
export function HospitalCard({ hospital }: HospitalCardProps) {
  return (
    <Link
      ...
      className="group flex h-full flex-col rounded-[28px] border border-[var(--border)] bg-white/92 p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-[var(--primary)]"
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.2em] text-[var(--primary)]">
            {hospital.availabilityStatus === "free" ? "Available" : "Busy"}
          </p>
        ...
        <div className="flex items-center gap-2 rounded-full bg-[var(--accent-soft)] px-3 py-2 text-sm font-semibold text-[var(--accent)]">
